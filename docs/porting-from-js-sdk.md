...
| `Client` | `packages/sdk/src/client/client.ts` | Rust currently covers option defaults, generated/explicit client keys, activate/deactivate over a transport boundary, deactivated status, sync/watch condition flags, sync modes, lifecycle/attachment option types, document attachment preconditions, and local attachment bookkeeping. Concrete network lifecycle must still follow JS. |
| `ClientTransport` | `packages/sdk/src/client/client.ts`, `client/client.go` | Rust transport boundary for activate/deactivate request fields, including shard key construction. Concrete Connect/protobuf transport is still pending. |
