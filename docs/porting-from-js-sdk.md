...
| `CrdtTree` | `packages/sdk/src/document/crdt/tree.ts` | Internal CRDT tree element with node lookup, serialization, data-size accounting, deep copy, GC-pair discovery, split-free element insert/delete edit behavior, text-node split insert/delete edit behavior, simple `splitLevel=1` element split behavior, and text-boundary split style/remove-style behavior. |
...
| `TreeEditOperation` | `packages/sdk/src/document/operation/tree_edit_operation.ts` | Operation that edits tree content. Rust currently covers split-free element insert/delete, text-node split insert/delete, simple `splitLevel=1` element split, operation info, insert/delete reverse operations, and tree-node GC registration. |
