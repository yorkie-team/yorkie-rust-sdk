...
| `TreeEditOperation` | `packages/sdk/src/document/operation/tree_edit_operation.ts` | Operation that edits tree content. Rust currently covers split-free element insert/delete, text-node split insert/delete, multi-level element split, visible-boundary merge, operation info, insert/delete reverse operations, and tree-node GC registration. |
