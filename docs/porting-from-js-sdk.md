...
| `Client` | `packages/sdk/src/client/client.ts` | Rust currently covers option defaults, generated/explicit client keys, activate/deactivate/attach/detach/remove/sync over a transport boundary, attach response metadata application, deactivated status, sync/watch condition flags, sync modes, lifecycle/attachment option types, document attachment preconditions, and local attachment bookkeeping. Concrete network lifecycle must still follow JS. |
| `ClientTransport` | `packages/sdk/src/client/client.ts`, `client/client.go` | Rust transport boundary for activate/deactivate/document attach/detach/remove and client-side push-pull request and response fields, including shard key construction, `ChangePack` handoff, removed `ChangePack` flag, document ID, max-size metadata, schema rules, and push-only mode. Concrete Connect/protobuf transport is still pending. |
