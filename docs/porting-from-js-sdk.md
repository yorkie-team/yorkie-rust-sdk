...
| `CrdtTree` | `packages/sdk/src/document/crdt/tree.ts` | Internal CRDT tree element with node lookup, serialization, data-size accounting, deep copy, GC-pair discovery, split-free element insert/delete edit behavior, text-node split insert/delete edit behavior, and text-boundary split style/remove-style behavior. |
