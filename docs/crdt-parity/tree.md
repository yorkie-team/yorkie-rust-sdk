...
| Node JSON/XML/data size | partial | Rust covers root/element/text serialization, UTF-16 text size, active attribute size, hidden removed children, text-node split size deltas during tree edit/style operations, and simple element split size deltas. Merge size deltas are not implemented yet. |
| CRDT tree element | partial | `CrdtElement::Tree` delegates metadata, JSON, data size, removal, deep copy, split-free element insert/delete edit behavior, text-node split insert/delete edit behavior, text-boundary split style/remove-style behavior, and simple `splitLevel=1` element split behavior. |
...
| Tree edit operation | partial | Rust has `TreeEditOperation` for split-free element insert/delete, text-node split insert/delete, simple `splitLevel=1` element split, reverse operation creation for insert/delete, op info, and tree-node GC registration. Multi-level split, merge, pure-split reverse operations, full `insPrevID`/`insNextID` maintenance, and unknown split sibling handling are still missing. |
...
- Extend tree style operation tests from text-boundary split toward
  ...
  propagation.
- Extend tree edit operation tests from simple `splitLevel=1` element split
  toward multi-level split, merge, pure-split reverse operations, and
  concurrent split sibling cases before public facade tests.
