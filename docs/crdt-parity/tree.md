...
| CRDT tree element | partial | `CrdtElement::Tree` delegates metadata, JSON, data size, removal, deep copy, initial style/remove-style behavior, and split-free element insert/delete edit behavior. |
| Tree style operation | partial | Rust has `TreeStyleOperation` for element attribute set/remove, reverse operation creation, op info, and removed attribute GC registration. It covers simple visible element-token ranges but does not yet split text, advance unknown split siblings, or propagate style across split siblings. |
| Tree edit operation | partial | Rust has `TreeEditOperation` for split-free element insert/delete, reverse operation creation, op info, and tree-node GC registration. Text split, element split, merge, `insPrevID`/`insNextID` maintenance, and unknown split sibling handling are still missing. |
...
| Tree GC | partial | Removed tree nodes and removed tree attributes are registered and purged through root GC when rebuilding from an existing root object. Tree style operations register removed attribute GC pairs; split-free tree edit operations register removed tree-node GC pairs. |
...
- Extend tree style operation tests around version-vector-aware styling,
  unknown split siblings, and split text once Tree edit/split exists.
- Extend tree edit operation tests from simple element insert/delete toward
  text split, element split, merge, and concurrent split sibling cases before
  public facade tests.
