...
| Snapshot GC rebuild | partial | Array moved/dead positions and text/tree internal pairs rebuild through copied root objects. Full `JSONElement` protocol conversion can carry those internal nodes, object GC elements survive core wire roundtrip, snapshot-root `ChangePack` protobuf bytes restore decoded roots, and decoded snapshot roots can now replace a document root through `Document::apply_change_pack`. Snapshot GC and presence behavior still need sync-level parity tests. |
