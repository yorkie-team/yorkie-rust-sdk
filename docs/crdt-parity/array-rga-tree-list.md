...
| Wire conversion | partial | Add/move/remove/array-set operations and full array `JSONElement` payloads convert to/from protobuf-shaped wire values. Rust ports Go's standalone array bytes scenario, the array portion of JS's root bytes scenario at core wire level, and a mixed protocol `ChangePack` replay covering add/remove/move. External binary fixtures are still missing. |
...
- Extend protocol-level replay fixtures from the current add/remove/move case
  toward duplicate position IDs, moved positions, dead positions, and GC.
