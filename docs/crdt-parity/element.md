...
| Dispatch to primitive/object/array/text/counter/tree | partial | Primitive, text, counter, and tree delegation are covered. Wire conversion now covers simple and full element payloads; public facade coverage is still uneven. |
...
| Wire conversion | partial | Simple elements and full `JSONElement` payloads convert to/from protocol-shaped wire values for primitive, object, array, text, counter, and tree. Snapshot application and cross-language binary fixtures are still missing. |

## Next Checks

- Add cross-language wire conversion fixtures for each element kind.
